...
def convert_color_space_Lab_to_RGB(img_Lab):
    ...
    matrix2 = [[np.sqrt(3)/3.0,0,0],[0,np.sqrt(6)/6.0,0],[0,0,np.sqrt(2)/2.0]]
    matrix3 = np.dot(matrix1,matrix2)
    #print(matrix3)
...
###########################################
    lstar_src = l - np.mean(l) 
    astar_src = a - np.mean(a)
    bstar_src = b - np.mean(b)
    ...
    ldash_new = ldash1 + np.mean(l_tar)
    adash_new = adash1 + np.mean(a_tar)
    bdash_new = bdash1 + np.mean(b_tar)
...
if __name__ == "__main__":
 ...
 #   path_file_image_result_in_CIECAM97s = sys.argv[5]
    ...
    cv2.imwrite('path_file_image_result_in_Lab1.png', img_BGR)
