...
def convert_color_space_RGB_to_Lab(img_RGB):
    ...
    constm2 = [[1/math.sqrt(3),0,0],[0,1/math.sqrt(6),0],[0,0,1/math.sqrt(2)]]
    ...
    for i in range(row):
        for j in range(col):
            ...
            LMSlog = np.log10(LMSm)

            LAB = np.matmul(constm4,LMSlog)
            img_Lab[i,j] = LAB
...
def convert_color_space_Lab_to_RGB(img_Lab):
    ...
    constm2 = [[math.sqrt(3)/3.0,0,0],[0,math.sqrt(6)/6.0,0],[0,0,math.sqrt(2)/2.0]]
    ...
    for i in range(row):
        for j in range(col):
            Labm  = img_Lab[i,j]
            LMSm = np.matmul(constm3, Labm)
            LMS10 = np.power(10,LMSm)

            RGB = np.matmul(constm4,LMS10)
            img_RGB[i,j] = RGB
