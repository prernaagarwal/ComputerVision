...
def warpimages(iflow, frame0, frame1, occ0, occ1, t):
    ...
    for y in range(h):
        for x in range(w):
            ...
            y0 = int(np.round(locx0[0]))
            x0 = int(np.round(locx0[1]))
            
            y1 = int(np.round(locx1[0]))
            x1 = int(np.round(locx1[1]))
             
            bilinear0 = 0
            a = locx0[0] - y0
            b = locx0[1] - x0
            if y0 + 1 < frame0.shape[0]  and x0 + 1 < frame0.shape[1]:
                bilinear0 = ( (1-a)*(1-b))*(frame0[y0, x0]) + (a*(1-b))*(frame0[y0, x0+1]) + (a*b)*(frame0[y0+1, x0+1]) + ((1-a)*b)*(frame0[y0+1, x0])

            bilinear1 = 0
            a = locx1[0] - y1
            b = locx1[1] - x1
            if y1 + 1 < frame1.shape[0]  and x1 + 1 < frame1.shape[1]:
                bilinear1 = ( (1-a)*(1-b))*(frame1[y1, x1]) + (a*(1-b))*(frame1[y1, x1+1]) + (a*b)*(frame1[y1+1, x1+1]) + ((1-a)*b)*(frame1[y1+1, x1])
            

            if (y0 < 0 or y0 > frame0.shape[0] or x0 < 0 or x0 > frame0.shape[1]):
                iframe[y][x] = bilinear1
                #iframe[y][x] = frame1[y1][x1]
            
            elif(y1 < 0 or y1 > frame1.shape[0] or x1 < 0 or x1 > frame1.shape[1]):
                iframe[y][x] = bilinear0
                #iframe[y][x] = frame0[y0][x0]

            if(occ0[y][x] == 0 and occ1[y][x] ==0):     #blend
                
                iframe[y][x] = (1 - t) * bilinear0 + t * bilinear1
