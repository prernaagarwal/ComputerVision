...
class SLIC:
    ...
    def generateSuperPixels(self):
        ...
        for i in range(self.ITERATIONS):
            ...
            for j in range(self.centers.shape[0]):
                xlow, xhigh = int(self.centers[j][3] - self.S), int(self.centers[j][3] + self.S)
                ylow, yhigh = int(self.centers[j][4] - self.S), int(self.centers[j][4] + self.S)
                ...
                dist = ((colorDist/self.m)**2 + (pixdist/self.ns)**2)**0.5
    ...
    def _initData(self):
        ...
        centers = []
        for i in range(self.S, self.width - self.S//2, self.S):
            for j in range(self.S, self.height - self.S//2, self.S):
                
                m = self._findLocalMinimum(center=(i, j))
                color = self.labimg[m[1], m[0]]
                center = [color[0], color[1], color[2], m[0], m[1]]
